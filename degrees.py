    ...
    while not queue.empty():
        ...
        for movie, neighbor in neighbors:
            if neighbor not in already_checked:
                ...
                new_node = Node(neighbor, current_node, movie)
                if neighbor == target:
                    return get_answer_array(new_node)
                queue.add(new_node)
    
    return None


def get_answer_array(node):
    answer = []
    while node.parent != None:
        print(node.state, node.action, node.parent.state)
        answer.insert(0,(node.action, node.state))
        node = node.parent
    return answer
